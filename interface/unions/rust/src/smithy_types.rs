            ...
            n => {
                return Err(RpcError::Deser(format!(
                    "invalid field number for union \
                     'org.wasmcloud.example.smithy_types#DataType':{}",
                    n
                )));
            }
        }
...
pub struct DocRef<'v> {
    #[serde(rename = "docRef")]
    pub doc_ref: wasmbus_rpc::common::DocumentRef<'v>,
}

// Encode DocRef as CBOR and append to output stream
#[doc(hidden)]
#[allow(unused_mut)]
pub fn encode_doc_ref<W: wasmbus_rpc::cbor::Write>(
    mut e: &mut wasmbus_rpc::cbor::Encoder<W>,
    val: &DocRef,
) -> RpcResult<()> {
    e.map(1)?;
    e.str("docRef")?;
    wasmbus_rpc::common::encode_document_ref(e, &val.doc_ref)?;
    Ok(())
}
